...
int main(int argc, char *argv[])
{
    ...
    result = eglQueryDevicesEXT(MAX_DEVICES, devices, &numDevices);
    assert(result != EGL_FALSE );
    fprintf(stdout, "number of Devices found %d\n", numDevices);
    EGLDisplay eglDpy = eglGetPlatformDisplayEXT(EGL_PLATFORM_DEVICE_EXT, devices[0], 0);
#endif

    EGLint major=0, minor=0;

    result = eglInitialize(eglDpy, &major, &minor);
    assert(result != EGL_FALSE );
    fprintf(stderr,"detected: EGL : %d.%d on display %p\n", (int)major, (int)minor, eglDpy);
    if (major <= 1 && minor < 4)
...
// first we call getConfigs with a NULL to see how many configs we have
    result = eglGetConfigs(eglDpy, NULL, 0, &numConfigs);
    assert(result != EGL_FALSE );
    fprintf(stdout, "number of configs found %d\n", numConfigs);

    result = eglChooseConfig(eglDpy, configAttribs, &eglCfg, 1, &numConfigs);
    assert(result != EGL_FALSE );
    fprintf(stdout, "number of configs matching our request %d\n", numConfigs);
